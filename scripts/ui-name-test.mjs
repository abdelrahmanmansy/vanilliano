...
// سلة فيها منتج عشان الشيك-اوت يفتح + تسجيل دخول تجريبي
await page.evaluate(() => {
...
let emailVal = ''
try { emailVal = await emailInput.inputValue() } catch {}
console.log('2) خانة الإيميل فاضية؟', emailVal.trim() === '' ? 'نعم ✅' : 'لا ❌ مكتوب: ' + emailVal)

// محاولة إتمام بدون اسم أو إيميل → لازم يمنع
